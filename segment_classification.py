...
if add_gaussian_noise_to_features:
    ...
    X = X + np.random.normal(0.0, sigma_noise, size=X.size).reshape(
        np.shape(X))
...
if variant == 'OneSlackSSVM':
    ...
    ssvm = FrankWolfeSSVM(model=model, C=5.e+4, tol=0.001, verbose=0,
                          max_iter=500)
else:
    assert False
...
for train_index, test_index in kf:
    ...
    print('SVM training of 1 fold has taken {} seconds'.format(
        (end - start) / 1000.0))
...
if plot_coefficients:
    ...
    plt.xticks(np.arange(num_features) + 0.5,
               ['x0norm', 'y0norm', 'x1norm', 'y1norm', 'length', 'angle'],
               rotation=45)
    plt.colorbar()
    plt.show()
    ...
    plt.matshow(
        ssvm.w[num_labels * num_features:].reshape(num_labels, num_labels))
